        ...
        tabItems(
          ...
          tabItem(
                        tabName = "Background",
                            fluidRow(
                                      h2(
                                            "Business Objective:",style = "color: blue",tags$br()
                                          ),
                                     
                                       h3(
                                            "Particulate matter (PM ) has been recorded in Delhi to understand the air quality on an hourly basis.",tags$br(), tags$br(),
                                            "Our objective is to forecast the data based on input data",tags$br(),tags$br()
                                          ),
                                     
                                      h4(
                                            ">> AQI is used by government agencies to communicate to the public how polluted  the air currently is or how polluted it is forecast to become in upcoming days",
                                            tags$br(),tags$br(),
                                            ">> Public health risks increase as the AQI rises.Air quality measurement are commonly reported in terms of micrograms per cubic meter parts per million or parts per billion",
                                            tags$br(),tags$br(),
...
server <- function(input, output,session)   {
  ...
  data <- eventReactive(input$file1,{    rnorm(1:100000)  })
    ...
    src = "https://blissair.com/wp-content/uploads/2013/06/pm25_comparison.jpg"
    output$PM25picture<-renderText({c('<img src="',src,'">')})
  
    output$Preview <- renderDataTable({
                                        Preview <- data.frame(date = c(seq(from = as.POSIXct("2018-01-01 06:30",tz="UTC"), 
                                           to = as.POSIXct("2018-01-01 15:30",tz="UTC"), by = "hour")),
    ...
    output$forecastplot <- renderPlot({
                                        ...
                                        ggplot()+
                                            ...
                                            geom_line(data = pred,aes(x=Date, y=`Point Forecast`,color="Predictions")) +
